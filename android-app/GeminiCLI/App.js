...
export default function App() {
  ...
  const initializeApp = async () => {
    try {
      ...
      const selectedModel = aiModelsManager.getCurrentModel();
  ...
  const sendMessage = async () => {
    if ((!inputText.trim() && !selectedImage && !recordedAudio) || !currentConversationId) {
      return;
    }

    // التحقق من وجود نموذج AI محدد ومفتاح API الخاص به
    const activeModel = aiModelsManager.getCurrentModel();
    if (!activeModel) {
      Alert.alert('خطأ', 'يرجى اختيار نموذج AI أولاً من الإعدادات المتقدمة');
      setShowAdvancedSettings(true);
      return;
    }

    const apiKey = await aiModelsManager.getAPIKey(activeModel.provider);
    if (!apiKey) {
      Alert.alert('خطأ', `يرجى إدخال مفتاح API لـ ${activeModel.provider} في الإعدادات المتقدمة`);
      setShowAdvancedSettings(true);
      return;
    }
    ...
    try {
      ...
      // استخدام النموذج المحدد حالياً
      const response = await aiModelsManager.sendMessage(activeModel, parts);
      ...
      if (response.success && response.data) {
        ...
        // تحديث إحصائيات الاستخدام للنموذج
        aiModelsManager.updateUsageStats(activeModel.provider, activeModel.id);
        ...
        throw new Error(response.error || 'فشل في الحصول على رد من نموذج AI');
      }
    } catch (error) {
      console.error(`Error calling ${activeModel?.name || 'AI'} API:`, error);
      const errorMessage = {
        id: Date.now() + 1,
        text: `عذراً، حدث خطأ في معالجة طلبك باستخدام ${activeModel?.name || 'نموذج AI'}. يرجى التحقق من مفتاح API والمحاولة مرة أخرى.`,
        isUser: false,
        timestamp: new Date().toLocaleTimeString(),
        isError: true,
      };
      ...
      if (settings.notifications?.errorAlerts) {
        notificationManager.notifyAPIError(`خطأ في الاتصال بـ ${activeModel?.name || 'نموذج AI'}`);
      }
    } finally {
      setIsLoading(false);
    }
