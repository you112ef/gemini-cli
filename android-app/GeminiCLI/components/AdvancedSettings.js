...
import aiModelsManager from '../utils/aiModelsManager';
...
const AdvancedSettings = ({ visible, onClose }) => {
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      await aiModelsManager.loadSettings();
      const aiData = {
        providers: aiModelsManager.getProviders(),
        currentModel: aiModelsManager.getCurrentModel(),
        usageStats: aiModelsManager.getUsageStats(),
      };
      ...
      [
        { text: 'إلغاء', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            await aiModelsManager.resetUsageStats();
            await loadSettings();
  ...
  const saveApiKey = async (provider, apiKey) => {
    try {
      await aiModelsManager.setAPIKey(provider, apiKey);
      setSelectedApiKey('');
      setSelectedProvider('');
      await loadSettings();
      Alert.alert('تم', `تم حفظ مفتاح API لـ ${provider} بنجاح`);
    } catch (error) {
      Alert.alert('خطأ', 'فشل في حفظ مفتاح API');
    }
  };

  const selectModel = async (providerId, modelId) => {
    try {
      await aiModelsManager.setCurrentModel(providerId, modelId);
      await loadSettings();
      Alert.alert('تم', 'تم تغيير النموذج بنجاح');
    } catch (error) {
      Alert.alert('خطأ', 'فشل في تغيير النموذج');
    }
  };

  const testConnection = async (provider) => {
    try {
      const result = await aiModelsManager.testConnection(provider);
      if (result.success) {
        Alert.alert('نجح الاتصال', 'تم الاتصال بالنموذج بنجاح');
      } else {
        Alert.alert('فشل الاتصال', result.error || 'فشل في الاتصال بالنموذج');
      }
    } catch (error) {
    ...
    >
      <View style={styles.container}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabsContainer}>
          ...
          {renderTabButton('models', 'نماذج AI', '🧠')}
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          {loading ? (
            ...
            <>
              ...
              {activeTab === 'models' && (
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>إدارة نماذج الذكاء الاصطناعي</Text>
                  
                  {/* النموذج الحالي */}
                  <View style={styles.currentModelContainer}>
                    <Text style={styles.currentModelTitle}>النموذج النشط حالياً:</Text>
                    <Text style={styles.currentModelText}>
                      {aiSettings.currentModel ? 
                        `${aiSettings.currentModel.name} (${aiSettings.currentModel.provider})` : 
                        'لم يتم اختيار نموذج'}
                    </Text>
                  </View>

                  {/* قائمة النماذج المتاحة */}
                  {aiSettings.providers && Object.entries(aiSettings.providers).map(([providerId, provider]) => (
                    <View key={providerId} style={styles.providerSection}>
                      <View style={styles.providerHeader}>
                        <Text style={styles.providerTitle}>{provider.name}</Text>
                        <TouchableOpacity 
                          style={styles.testButton}
                          onPress={() => testConnection(providerId)}
                        >
                      ...
                      <View style={styles.apiKeySection}>
                        <Text style={styles.apiKeyTitle}>مفتاح API:</Text>
                        <View style={styles.apiKeyRow}>
                          <TextInput
                            style={styles.apiKeyInput}
                            placeholder={`أدخل مفتاح API لـ ${provider.name}`}
                            placeholderTextColor="#888"
                            value={selectedProvider === providerId ? selectedApiKey : ''}
                            onChangeText={(text) => {
                              setSelectedProvider(providerId);
                              setSelectedApiKey(text);
                            }}
                            secureTextEntry
                          />
                          <TouchableOpacity 
                            style={styles.saveApiButton}
                            onPress={() => saveApiKey(providerId, selectedApiKey)}
                            disabled={!selectedApiKey || selectedProvider !== providerId}
                          >
                            <Text style={styles.saveApiButtonText}>حفظ</Text>
                          </TouchableOpacity>
                        </View>
                      </View>

                      {/* النماذج المتاحة */}
                      <Text style={styles.modelsTitle}>النماذج المتاحة:</Text>
                      {provider.models.map((model) => (
                        <TouchableOpacity 
                          key={model.id}
                          style={[
                            styles.modelItem,
                            aiSettings.currentModel?.id === model.id && styles.activeModelItem
                          ]}
                          onPress={() => selectModel(providerId, model.id)}
                        >
                          ...
                          {aiSettings.currentModel?.id === model.id && (
                            <Text style={styles.activeIndicator}>✓</Text>
                          )}
                        </TouchableOpacity>
                      ))}

                      {/* إحصائيات الاستخدام للمزود */}
                      {aiSettings.usageStats && aiSettings.usageStats[providerId] && (
                        <View style={styles.providerStats}>
                          <Text style={styles.statsTitle}>إحصائيات الاستخدام:</Text>
                          <Text style={styles.statText}>
                            الرسائل: {aiSettings.usageStats[providerId].totalMessages || 0}
                          </Text>
                          <Text style={styles.statText}>
                            آخر استخدام: {aiSettings.usageStats[providerId].lastUsed ? 
                              new Date(aiSettings.usageStats[providerId].lastUsed).toLocaleDateString('ar') : 
                              'لم يستخدم بعد'}
                          </Text>
                        </View>
                      )}
              ...
              {activeTab === 'stats' && (
                <View style={styles.section}>
                  ...
                  {/* إحصائيات نماذج AI */}
                  {aiSettings.usageStats && Object.keys(aiSettings.usageStats).length > 0 && (
                    <View style={styles.aiStatsContainer}>
                      <Text style={styles.aiStatsTitle}>إحصائيات نماذج AI:</Text>
                      {Object.entries(aiSettings.usageStats).map(([provider, stats]) => (
                        <View key={provider} style={styles.aiStatItem}>
                          <Text style={styles.aiStatProvider}>{provider}:</Text>
                          <Text style={styles.aiStatText}>{stats.totalMessages || 0} رسالة</Text>
                        </View>
                      ))}
                    </View>
                  )}
